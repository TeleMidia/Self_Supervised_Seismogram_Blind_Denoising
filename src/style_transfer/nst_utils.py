...
class CONFIG:
    ...
    COLOR_CHANNELS = 1
    NOISE_RATIO = 0.6
    MEANS = np.array([128]).reshape((1,1,1,COLOR_CHANNELS)) 
    VGG_MODEL = 'pretrained-model/svgg-verydeep-19.mat' # Pick the VGG 19-layer model by from the paper "Very Deep Convolutional Networks for Large-Scale Image Recognition".
...
def load_vgg_model(path):
    ...
    def _conv2d(prev_layer, layer, layer_name):
        ...
        if layer == 0:
            W_aux = np.zeros(shape=(W.shape[0],  W.shape[1], 1,  W.shape[3]), dtype = float)
...
def restore_image(image):
    ...
    #print("final image", image.shape) 

    return image/255.0
    #scipy.misc.imsave(path, image)

def save_tifffile(path, filename, tiff_data):
